...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <form method="post" action="/admin/package/{{$package->id}}/update" enctype="multipart/form-data">
            <div class="col-xxl">
                <div class="card mb-4">
                    <div class="card-body">
                            @csrf
                            ...
                            <div class="d-flex gap-3 mb-3 w-100">
                                <div class="w-100">
                                    ...
                                    <select class="form-select" name="type" id="exampleFormControlSelect1"
                                        ...
                                        <option {{ $package->category == 'BUMN' ? 'selected' : '' }} value="BUMN">BUMN
                                        </option>
                                        <option {{ $package->category == 'CPNS' ? 'selected' : '' }} value="CPNS">CPNS
                                        </option>
                                    </select>
                                </div>
                                <div class="w-100">
                                    ...
                                    <select class="form-select" name="category" id="exampleFormControlSelect1"
                                        aria-label="Default select example">
                                        <option {{ $package->tipe == 'try-out' ? 'selected' : '' }} value="try-out">Try Out
                                        </option>
                                        <option {{ $package->tipe == 'kursus' ? 'selected' : '' }} value="kursus">Kursus
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="w-100">
                                    ...
                                    <div class="input-group input-group-merge">
                                        ...
                                        <input type="text" name="price" onkeyup="rupiah(this)"
                                            id="basic-icon-default-company" placeholder="50.000"
                                            value="{{ $package->price }}" class="form-control" />
                                    </div>
                                </div>
                                <div class="w-100">
                                    ...
                                    <div class="input-group input-group-merge">
                                        <input type="text" name="discount" onkeyup="rupiah(this)"
                                            id="basic-icon-default-company" placeholder="50.000"
                                            value="{{ $package->discount }}" class="form-control" />
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="hidden" name="file_asli" value="{{$package->photo}}" id="">
                                                                ...
                                                                <div class="">
                                                                    <input type="number" class="form-control"
                                                                        @foreach ($setting as $s)
                                                                            @if ($s['id'] == $item->id)
                                                                            value="{{ $s['amount_acces'] }}"
                                                                            @endif
                                                                        @endforeach
                                                                        id="akses{{ $item['id'] }}" name="acces[]"
                                                                        placeholder="1" />
                                                                </div>
                                                            </div>
                                                            <div class="form-check">
                                                                <input class="form-check-input" type="checkbox"
                                                                    name="is_review[]" value="{{ $item['id'] }}"
                                                                        @foreach ($setting as $s)
                                                                            @if ($s['id'] == $item->id && $s['is_review'] == 1)
                                                                            checked
                                                                            @endif
                                                                        @endforeach
                                                                    id="pembahasan{{ $item['id'] }}">
                                                        ...
                                                        <td>
                                                            ...
                                                            <div class="form-check">
                                                                <input class="form-check-input checklist"
                                                                    {{ in_array($item['id'], $idExam) ? 'checked' : '' }}
                                                    ...
                                                    @if ($item->type == 'videos')
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div class="form-check">
                                                                    <input class="form-check-input" type="checkbox"
                                                                        {{ in_array($item['id'], $idCourse) ? 'checked' : '' }}
                                                                        name="video[]" value="{{ $item['id'] }}"
        ...
        function tampilVideo() {
            ...
            for (var i = 0; i < resultVideo.length; i++) {
                $(wraperTotal).append(
                    ...
                    '</td><td><iframe width="560" height="315" src="https://www.youtube.com/embed/' + resultVideo[i][
                    2] +
                    '?si=Y4ypGZqQSFM97EXN" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></td></tr>'
                );
                no++
            }
        }
        ...
        function tampilFile() {
            ...
            for (var i = 0; i < resultFile.length; i++) {
                ...
                $(wraperTotal).append(
                    ...
                    '</td><td><embed src="/show-pdf/' + fileName +
                    '" width="550px" height="500px" type="text/pdf"></td></tr>'
                );
                no++
            }
        }
