                    ...
                    <ol>
                        @if ($item->is_correct == 1)
                            @foreach ($item->questionDetail_questionId as $option) 
                                <li class="<?= $item->question_detail_id == $option->id  ? 'jawaban-benar' : '' ?> ">{{ $option->content_answer }}</li>
                            @endforeach
                        @else
                            @foreach ($item->questionDetail_questionId as $option) 
                                <li class="<?= $option->id == $item->question_detail_id ? 'jawaban-salah' : ''?> <?= $option->correct_answer == 1 ? 'jawaban-benar' : ''?> ">{{ $option->content_answer }}</li>
                            @endforeach
                        @endif
